...
export default async function QuotationDetailPage({ params }: { params: { id: string } }) {
  ...
  return (
    <div>
      ...
      <Card className="mt-4"><h2 className="mb-3 text-lg font-semibold">Line Items</h2><div className="overflow-x-auto"><table className="min-w-full text-sm"><thead><tr className="border-b border-slate-300 text-left text-slate-700"><th className="px-2 py-2">Description</th><th className="px-2 py-2">Qty</th><th className="px-2 py-2">Unit Price</th><th className="px-2 py-2">Line Total</th></tr></thead><tbody>{quotation.items.map((item) => <tr key={item.id} className="border-b border-slate-200"><td className="px-2 py-2">{item.description || item.phoneModel?.modelName || item.phoneVariant?.variantName || "Quotation item"}</td><td className="px-2 py-2">{item.quantity}</td><td className="px-2 py-2">{formatMoney(Number(item.unitPrice))}</td><td className="px-2 py-2">{formatMoney(Number(item.lineTotal))}</td></tr>)}{!quotation.items.length ? <tr><td colSpan={4} className="px-2 py-4 text-slate-600">No line items were added.</td></tr> : null}</tbody></table></div><div className="mt-5 ml-auto max-w-xs space-y-2 text-sm text-slate-700"><p className="flex justify-between"><span>Subtotal</span><span>{formatMoney(Number(quotation.subtotal))}</span></p><p className="flex justify-between"><span>Tax{quotation.taxType === "Percent" ? ` (${Number(quotation.taxPercent).toFixed(2)}%)` : ""}</span><span>{formatMoney(Number(quotation.taxAmount))}</span></p><p className="flex justify-between"><span>Handling fee</span><span>{formatMoney(Number(quotation.handlingFee))}</span></p><p className="flex justify-between border-t border-slate-300 pt-2 text-base font-semibold text-slate-950"><span>Total</span><span>{formatMoney(Number(quotation.totalAmount))}</span></p></div></Card>
    </div>
  );
}
